...
class UncertaintySampling(Strategy):
    def get_index_to_label_next(self, predictor, unlabeled_dataset):
        with torch.no_grad():
            ...
            index_to_label_next = int(unlabeled_dataset.indices[torch.argmin(certainty)])
        return index_to_label_next



def get_al_curve(predictor, strategy, train_dataset, test_dataset, initial_labeled_indices):
  ...
  while len(unlabeled_ds) > 0:  # fit, get point, label
    ...
    predictor = fit_predictor(predictor, loss_fn, optimizer, labeled_ds, verbose=False)
    performance = evaluate_predictor(predictor, eval_loss_fn, test_dataset, verbose=False)
