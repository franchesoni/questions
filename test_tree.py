...
from tree_and_state import get_node_with_state, initialize_tree, tree_search, update_state, ActionNode, StateNode
...
def visualize_tree(root_node, name):
    ...
    def traverse(node):
        ...
        graph.add_node(node_label, color=color, shape="box" if isinstance(node, ActionNode) else "ellipse")
...
def example_2d_gaussians(max_expansions, max_n):
    ...
    while len(state["labeled"]) < N:
        ...
        plot_model_outputs(prob_of_being_positive, pos_center, X, y, X_to_annotate, y_to_annotate, indices_to_ask, predictions, X_annotated, y_annotated, name=f"aia/report_step_{n_question}")
