...
from tree_and_state import get_node_with_state, initialize_tree, tree_search, update_state


def plot_model_outputs(model, X, y, X_to_annotate, y_to_annotate, indices_to_ask, predictions, X_annotated, y_annotated, name):
  ...
  plt.contour(x1, x2, 1 / np.sqrt(2 * np.pi) * np.exp(-((x1 - 1) ** 2+(x2-1)**2) / 2), levels=3, colors="r")
...
def example_2d_gaussians(max_expansions, max_n):
    ...
    def prob_pos_fn(x):
        ...
        return 1 / (2 * np.pi) * np.exp(-0.5 * ((x[:, 0] - 1) ** 2 + (x[:, 1] - 1) ** 2))
    ...
    while len(state["labeled"]) < N:
        optimal_action, optimal_cost = tree_search(
            root_node,
            N=N,
        ...
        plot_model_outputs(prob_of_being_positive, X, y, X_to_annotate, y_to_annotate, indices_to_ask, predictions, X_annotated, y_annotated, name=f"aia/report_step_{n_question}")
        ...
        unlabeled_indices = [ind for ind in range(len(X)) if ind not in annotated_indices]
...
def visualize_tree(root_node, name):
    ...
    def traverse(node):
        ...
        node_label = str(node.state) if isinstance(node, StateNode) else (str(node.guess) + " at " + str(node.parent.state))
        node_object = pydot.Node(node_label, label=node_label, color="red" if isinstance(node, ActionNode) else "blue")
        if isinstance(node, ActionNode):
            node_object.set_shape("box")
        ...
        if node.parent:
            ...
            if isinstance(node.parent, ActionNode):
                state_prob = float(node.parent.children_probs[node.parent.state_children.index(node)][0])
                edge_label = f"prob = {state_prob:.2f}, cost = {float(node.cost):.2f}"
            else:
                edge_label = f"cost = {float(node.cost):.2f}"
            edge_object = pydot.Edge(parent_label, node_label, label=edge_label)
