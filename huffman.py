...
def analyse_huffman_tree(root_node, node_predictions):
    ...
    def traverse(node):
        ...
        if node.parent:
            if len(node.indices) == 1 or len(set(sorted_indices[:len(node.indices)]) - set(node.indices)) == 1:
                is_most_likely = True  # we assume n=1 doesn't count
            else:
                is_most_likely = (set(sorted_indices[:len(node.indices)]) == set(node.indices)) or (len(set(sorted_indices[:len(sorted_indices)-len(node.indices)]).intersection(set(node.indices))) == 0)
        else:
            is_most_likely = None
