...
def get_ia_curve(
    max_queries, predictor, update_predictor_every, train_dataset, test_dataset, initial_labeled_indices, sts_kwargs={}, dstfilename=None
):
    ...
    while n_queries < max_queries and len(unlabeled_ds) > 0:  # fit, get point, label
        if n_queries % update_predictor_every == 0:
            ### data update ###
        ...
        root_node = STS.set_new_root_node(best_question_node.state_children[answer*1], destroy_descendants=(n_queries + 1) % update_predictor_every == 0)
        # check for equality
        check = True
