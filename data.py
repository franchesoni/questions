...
def get_labeled_unlabeled_slow(train_dataset, labeled_indices):
  ...
  unlabeled_ds = torch.utils.data.Subset(EnumeratedUnlabeledWrapperSlow(train_dataset), unlabeled_indices)  # indices are needed for selection and we unlabel the dataset to prove we aren't cheating
...
def get_torchvision_dataset(dataset_name, binarizer=None):
    ...
    if dataset_name == 'cifar10':
        ...
        train_dataset = datasets.CIFAR10(download_path, download=True, train=True, transform=train_transform, target_transform=binary_transform)
        test_dataset = datasets.CIFAR10(download_path, download=True, train=False, transform=test_transform, target_transform=binary_transform)
        ...
        train_dataset = datasets.MNIST(download_path, download=True, train=True, transform=train_transform, target_transform=binary_transform)
        test_dataset = datasets.MNIST(download_path, download=True, train=False, transform=test_transform, target_transform=binary_transform)
        ...
        train_dataset = datasets.FashionMNIST(download_path, download=True, train=True, transform=train_transform, target_transform=binary_transform)
        test_dataset = datasets.FashionMNIST(download_path, download=True, train=False, transform=test_transform, target_transform=binary_transform)
        ...
        train_dataset = datasets.SVHN(download_path, download=True, split='train', transform=train_transform, target_transform=binary_transform)
        test_dataset = datasets.SVHN(download_path, download=True, split='test', transform=test_transform, target_transform=binary_transform) 
        ...
        train_dataset = datasets.STL10(download_path, download=True, split='train', transform=train_transform, target_transform=binary_transform)
        test_dataset = datasets.STL10(download_path, download=True, split='test', transform=test_transform, target_transform=binary_transform)
...
class InMemoryDataset:
    def __init__(self, torchvision_dataset, binarizer_fn=None, use_only_first=False):
        ...
        self.data = self.data.permute(0, 3, 1, 2) if not torchvision_dataset.__class__.__name__ in ['SVHN', 'STL10'] else self.data
        self.targets = torchvision_dataset.targets if hasattr(torchvision_dataset, 'targets') else torchvision_dataset.labels
